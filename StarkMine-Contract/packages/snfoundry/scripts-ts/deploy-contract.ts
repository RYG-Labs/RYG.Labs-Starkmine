...
if (!deployer) {
  throw new Error(
    red(`\n[deploy-contract.ts] 'deployer' is undefined for network '${networkName}'.\n\nThis usually means one or more required environment variables are missing or incorrect.\n\nPlease ensure the following environment variables are set for the '${networkName}' network:\n\n  - ACCOUNT_ADDRESS_${networkName.toUpperCase()}\n  - PRIVATE_KEY_${networkName.toUpperCase()}\n  - RPC_URL_${networkName.toUpperCase()}\n\nYou can set them in your shell, .env file, or CI/CD environment.\n`)
  );
}
