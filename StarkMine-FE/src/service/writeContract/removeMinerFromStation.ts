...
const removeMinerFromStation = async (account: AccountInterface, stationId: number, minerSlot: number ): Promise<MessageBase> => {
    try {
        const tx = await account.execute({
            contractAddress: contracts.StationSystem,
            entrypoint: "remove_miner_from_station",
            calldata: CallData.compile({ station_id: stationId, miner_slot: minerSlot }),
        });
